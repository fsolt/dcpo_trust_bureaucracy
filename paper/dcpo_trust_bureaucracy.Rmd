...
<!--Niger in input date only has two time points. The question is about tax.2015's data of trust in institutions  was higher than data in later years https://bti-project.org/en/reports/country-report/NER. Most African countries in top 20 only have trust_tax at two time points from Afro5 and 6, including Niger, Namibia, Senegal, Botswana, Malawi, Mauritius, and Mozambique. -->
...
```{r construct_validation_survey, cache = TRUE, cache.extra = tools::md5sum(here::here("data", "tcs.rda")), fig.cap="Construct Validation: Correlations Between TCS Scores and Other Variables \\label{surv_const_plot}", fig.height = 4.5,fig.width= 8}
...
tcs_de_plot <- court_de %>% 
  ...
  geom_segment(aes(x = year, xend = year,
               ...
               na.rm = TRUE,
               alpha = .2) +
  annotate("text", x = 2009, y = .30, size = 2,
...
tnatgov_plot | ((tcourt_plot + tcs_de_plot)/(tcs_cor_plot + tcs_sat_plot)) +
 patchwork::plot_annotation(tag_levels = 'A', 
                            caption = "Note: Gray whiskers and shading represent 80% credible intervals.")  #why tag does not work. 
