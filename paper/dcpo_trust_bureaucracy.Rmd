...
@Morelock2021's study also highlights the positive role of government effectiveness, although @Houston2016's study find inconsistent role of government effectiveness.
...
To minimize the noise from the sparse data and increase comparability, drawn from the raw data, we followed a common approach [@Woo2023a] and filtered out `r n_items` survey items that were asked in no fewer than five country-years in countries surveyed at least twice.^[
...
Ideally, if all countries were surveyed annually, we would have `r total_cy` observations per year and a total of `r {n_countries * n_years * n_items} %>% scales::comma()` country-year-item observations.
However, the actual dataset is far more limited, with only `r n_cy` country-years containing at least some data on trust in civil servants. This accounts for `r round(covered_share)` of the `r spanned_cy` country-years spanned by our dataset.
...
As depicted in the upper left panel of Figure \nobreakspace{}\ref{item_country_plots}, the Germany, with `r ge_obs` country-year-item observations, is the most represented country, followed by `r others`.
The upper right panel expands on this by listing the twelve countries with the highest number of years observed, revealing overlaps and differences from the previous group.
Countries such as `r adding`join the list, replacing `r dropping`, indicating a consistent interest in surveying trust-civil-servants-related questions in these countries.
...
To estimate trust in civil servants across countries and over time, we employ the recent and suitable method for handling data that is both incomparable and sparse: the Dynamic Comparative Public Opinion (DCPO) model built in @Solt2020c.^[
...
The DCPO model, a population-level two-parameter ordinal logistic item response theory (IRT) model, address the two principal challenges posed by our source data: incomparability and sparsity.
...
The extent to which each response reflects the latent trait is most clearly demonstrated through ordinal responses to the same question: strongly agreeing with the statement "Most government administrators (civil servants) can be trusted to do what is best for the country," exhibits more trust in civil servants than simply agreeing, which shows more trust than responding "disagree," which in turn is a more trusting response than "strongly disagree."
This logic extends across different questions as well.
...
To address the sparsity of the source data, characterized by gaps in the time series for each country and many country-years with only a single available item, the DCPO model employs random-walk priors for each country.
This means that within each country, a given year's trust level is modeled as the previous year's estimate plus a random shock.
These random-walk priors smooth trust estimates over time, allowing the generation of estimates even for years with little or no data, albeit with greater measurement uncertainty.
For more information on the DCPO model, see Appendix B and @Solt2020c [, 3-8].
...
Figure\nobreakspace{}\ref{ts_plots} also shows that while TCS has risen prominently in some countries, such as Finland and Germany, trusting attitudes have maintained high or low over time in others, like Luxembourg and Mexico, or fallen steadily, as in U.S. and Chile. 
They have advanced and retreated as in Spain or have declined and recovered as in Turkey.
The differences within countries over time and differences across countries present a challenge to explain the causes and consequences of trust in bureaucracies. 
...
The general strong relationship between TCS measures and institutional trust confirms the theoretical expectation about institutional trust, but the variation in TCS' relationship with trust in different institutions cross regions is worthwhile to further investigate to refine the dimensions in political trust. 
...
We then conduct construct validation in Figure\nobreakspace{}\ref{ext_val2}.
...
Corruption is often often argued as a likely contributor to trust in civil servants and administration [see, e.g., @Anderson2003; @VanRyzin2011;@VandeWalle2022].
The left panel compares perceived widespread of corruption, measured as the percentage of those saying most or state authorities are involved in corruption in seven WVS surveys, with the TCS scores.
As theoretical expectation, there is a clear negative relationship between the spread of perceived corruption and TCS: when there is widespread perception of corruption in authorities, the public tends to distrust civil servants.
The center and right panel of Figure\nobreakspace{}\ref{ext_val2} also present negative relationships between TCS scores and the spread of perceived corruption of government officials cross different geographic regions.
The center panel shows the data in developing or new democratic countries surveyed in Asian, New Europe Barometer, and Latinobarometro, and the right panel displays the data in countries surveyed in The International Social Survey Programme Citizenship module (2004, 2014).

To sum up, the evidence of construct validation of TCS scores agains the perceived widespread of corruption in Figure\nobreakspace{}\ref{ext_val2}, together with the evidence of external validation in Figure\nobreakspace{}\ref{ext_val1} and convergent validation in Figure \nobreakspace{}\ref{internal_val}, demonstrates the validity of the TCS scores as measures of the public's trust in civil servants. 
...
To measure income inequality, we relied on @Solt2020's Standardized World Income Inequality Database (SWIID), specifically using the index of inequality in disposable income for its superior coverage and comparability.
...
We also focused on the Government Effectiveness indicator from the World Bank's Worldwide Governance Indicators, as it encompasses the overall quality of public services, the civil service, and policy formulation and implementation.
...
To capture both short-term and historical effects of predictors, we employed the 'within-between random effects' specification. This approach incorporates the time-invariant country mean and the time-varying difference between each country-year value and this country mean, following @Bell2015 and @Woo2023.
...
```{r resplot, fig.cap="Predicting Trust in Bueacracies \\label{model}", fig.height=6, fig.width=7.5}
...
named_vars <- tribble(~var_name, ~`.variable`, ~order,
                      ...
                      "Level of Liberabl Democracy, Mean", "b_libdem_mean",15,
                      "Level of Liberabl Democracy, Difference",  "b_libdem_diff",16)
...
coef_data0_all %>% 
  ...
  xlab(NULL) +
  ylab(NULL) +
  plot_annotation(caption = "Notes: Dots indicate posterior means; whiskers, from thickest to thinnest, describe \n95% credible intervals; shading depicts the posterior probability density function.")
```
...
A two-standard-deviation year-to-year change increases trust by `r get_coef("b_gdppc_diff", results_df = coef_data_all) %>% str_remove("-")` point (95% c.i.:`r get_coef("b_gdppc_diff", results_df = coef_data_all, type = "ci")`).
Similarly, we find that a two-standard-deviation year-to-year increase in unemployment rate in a short run decreases trust in civil servants by `r get_coef("b_unemploy_diff", results_df = coef_data_all) %>% str_remove("-")` point (95% c.i.:`r get_coef("b_unemploy_diff", results_df = coef_data_all,type = "ci")`).
Regarding outcomes in public safety, government's performance in public security measured by the number of intentional homicides at the country-year level has a "historical", long-run negative effect on trust in civil servants.
A two-standard deviation increase in a country's mean public insecurity is associated with `r get_coef("b_victims_mean", results_df = coef_data_all) %>% str_remove("-")` points less trust (95% credible interval: `r get_coef("b_victims_mean", results_df = coef_data_all,type = "ci")` points) across all countries.
However, we do not find evidence for the either "historical" or short-run effect of inflation and income inequality on trust in civil servants.

With regards of government quality-or process, a higher government effectiveness score was found to have a strong positive long-term effect on trust in civil servants.
A two-standard deviation increase in a country's mean effectiveness score is associated with `r get_coef("b_gee_mean", results_df = coef_data_all) %>% str_remove("-")` points more trust (95% credible interval: `r get_coef("b_gee_mean", results_df = coef_data_all,type = "ci")` points) across all countries.
The development of democracy is associated with less trust in civil servants, both in the long run and in the short term.
Critical citizens in more democratic countries could trust civil servant critically and have a higher demand for them [@Norris1999].
However, perceived Corruption measured by Corruption perception index was not associated with trust in civil servants across countries at 95% confident interval at the both long-term and short-run and estimates are close to zero.

We find evidence for both arguments on government outcomes and government quality. 
Among predictors, compared to economic and public security outcomes, such as year-to-year GDP per capita, unemployment and countries' mean insecurity, government quality or process, measured by countries' mean government effectiveness and mean democratic development, has more substantive effects in long run.
...
For example, Asian countries with a history of meritocracy and rapid economic development, such as China and Singapore, as well as least corrupt countries like Switzerland and Norway, exhibit high levels of trust in civil servants.
...
However, long-term trust is more strongly enhanced by government quality and effectiveness in service delivery, policy formation, and implementation.

While this study focuses on the determinants of trust in bureaucracy, the publicly accessible Trust in Civil Servants (TCS) dataset we provide opens opportunities for future research to explore the consequences of trust in civil servants.
