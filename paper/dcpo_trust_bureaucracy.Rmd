...
Specifically, the lack of comparative data on trust on bureaucracy makes it impossible to test the competing theories about what affects trusting attitudes, government performance, or the quality of governance [@Bouckaert2012; @Kettl2000; @Van2022; @Morelock2021].

```{r dcpo_input_raw, include=FALSE, cache.extra = tools::md5sum(here::here("data-raw", "surveys_bureaucracy.csv"))}
...
```{r tb_summary_stats, cache.extra = tools::md5sum(here::here("data-raw", "dcpo_input_raw.csv"))}
...
```{r dcpo_input, eval=evaluate, cache=TRUE, dependson="dcpo_chunk_options", include=FALSE, results=FALSE}
...
```{r dcpo, eval=evaluate, cache=evaluate, dependson="dcpo_input", include=FALSE, results=FALSE}
 iter <- 1000
...
As displayed in Figure&nbsp;\ref{cs_mry}, the dataset covers a wide range of geographic breadth, allowing comparative studies of countries and regions too often neglected [see @Wilson2021].
...
```{r construct_val, fig.height = 4,fig.width= 8.5, fig.cap = "Construct Validation for Trust Civil Servants . \\label{tcs_ev}"}
