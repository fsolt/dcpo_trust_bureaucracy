...
```{r dcpo_input_raw, include=FALSE, cache=TRUE}
...
```{r itemcountry, fig.cap="Countries and Years with the Most Observations in the Source Data \\label{item_country_plots}", fig.height=3.5, fig.pos='h'}
...
The upper left panel of Figure\nobreakspace{}\@ref(fig:item_and_country_plots) shows the dozen countries with the highest count of country-year-item observations.
...
```{r dcpo_input, eval=TRUE, include=FALSE, results=FALSE}
...
```{r dcpo, eval=TRUE, include=FALSE, results=FALSE}
 iter <- 1000
...
Figure\nobreakspace{}\@ref(fig:cs_mry) displays the most recent available TCS score for each of the `r res_c` countries and territories in the dataset.
...
We show the changes of TCS over time in sixteen countries in Figure\nobreakspace{}\@ref(fig:ts_plots).
As displayed in Figure\nobreakspace{}\@ref(fig:cs_mry), the dataset covers a wide range of geographic breadth, allowing comparative studies of countries and regions too often neglected [see @Wilson2021].
Figure\nobreakspace{}\@ref(fig:ts_plots) also shows that while TCS has risen prominently in some countries, such as Finland and U.K., trusting attitudes have maintained high or low over time in others, like China and Turkey, or fallen steadily, as in U.s. and Mexico. 
...
```{r tcs_internal_val, fig.height = 4, fig.cap = "Convergent Validation: Correlations Between TSC Scores and Individual TSC Source-Data Survey Items"}
...
```{r tcs_ext_val1, fig.cap="Construct Validation: Correlations Between TCS Scores and Trust in Institutions Survey Items", fig.height=4}

ext_wvs_gov_dat <- tibble(survey = c("wvs_combo", "wvs7"),
                          item = "trust_gov4",
                          variable = c("e069_11", "q71"),
                          values = "c(4:1)") %>% 
  DCPOtools::dcpo_setup() %>%  
  mutate(title = "World Values Survey",
         neg = FALSE)

ext_lb_courts_dat <- read_csv(here:: here("data-raw",
                                       "surveys_courts.csv")) %>% 
  DCPOtools::dcpo_setup() %>%  
  mutate(title = "Latinobarómetro",
         neg = FALSE) 

ext_evs_parl_dat <- read_csv(here:: here("data-raw",
...
```{r tcs_ext_val2, fig.height = 4, fig.cap = "Construct Validation: Correlations Between TCS Scores and Corruption of Public Servants Survey Items"}

ext_wvs_cor_dat <- tibble(survey = c("wvs_combo", "wvs7"),
                          item = "corrupt_state4",
                          variable = c("e269", "q113"),
                          values = "c(1:4)") %>% 
  DCPOtools::dcpo_setup() %>%  
  mutate(title = "World Values Survey",
         neg = FALSE)

ext_cor_dat <- read_csv(here:: here("data-raw",
                     "surveys_cor.csv")) %>% 
  DCPOtools::dcpo_setup() %>%  
  mutate(title = "Arab, Asian & New Europe Barometers\nand Latinobarómetro",
         neg = FALSE)

ext_issp_cor_dat <- tibble(survey = c("issp2004", "issp2014"),
                          item = "corrupt5",
                          variable = c("v59", "v61"),
                          values = "c(1:5)") %>% 
  DCPOtools::dcpo_setup() %>%  
