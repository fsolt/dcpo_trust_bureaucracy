...
24.2)),
    ...
    left_join(vdem13,
              by = c("country", "year")) %>% 
    left_join(qog_outcomes,
              by = c("country", "year")) %>%
    left_join(who_outcomes,
              by = c("country", "year")) %>%
...
data_combo_all <- data_combo %>%
  ...
  drop_na(mean:imf_gdppc, gini_disp:icrg_qog) %>%
    group_by(country) %>% 
    mutate(
           ...
           gini_diff_se = sqrt(gini_disp_se^2 + gini_mean_se^2),
...
named_vars <- tribble(~var_name, ~`.variable`, ~order,
                        ...
                        "Inflation (in log)", "b_log_inf", 3,
                        "Unemployment (in log)", "b_log_une", 4, 
                        "Security, Mean", "b_victims_mean", 5,
                        "Security, Difference", "b_victims_diff", 6,
                     "Level of Liberabl Democracy, Mean", "b_libdem_mean",7,
                      "Level of Liberabl Democracy, Difference",  "b_libdem_diff",8,
                      "Government Effectiveness, Mean", "b_gee_mean",9,
                      "Government Effectiveness, Difference", "b_gee_diff",10,
                         "Income Inequality, Mean", "bsp_megini_meangini_mean_se", 11,
                        "Income Inequality, Difference",
                                                  "bsp_megini_diffgini_diff_se", 12,
